...
(sécurité, stabilité, scalabilité) · `dev_ops:decisions/` (pourquoi, et D30 pour ce dépôt).

---

## 1. Ce que ce dépôt est — un dépôt léger (décision D30)
...
D'où une stack réduite (D30) : pas d'environnement GitHub, pas de compte de service, pas de workflow
...
(`--check` refuse une copie qui diverge). **Ne rien modifier dans `.claude/` sur place.** Directives
